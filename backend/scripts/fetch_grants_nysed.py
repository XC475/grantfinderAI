...
def extract_grant_urls(base_url: str) -> List[str]:
    """Extract all grant URLs from the main grants listing page."""
    logger.info(f"Fetching grant URLs from {base_url}")
    ...
    try:
        ...
        grant_urls = []
        # Look for links containing "funding-opportunities" and not containing procurements
        for link in soup.find_all('a', href=True):
            href = link['href']
            if '/funding-opportunities/' in href and 'procurement' not in href.lower():
                full_url = urljoin(BASE_URL, href) if href.startswith('/') else href
                if full_url not in grant_urls:
                    grant_urls.append(full_url)
        
        logger.info(f"Found {len(grant_urls)} potential grant URLs")
        return grant_urls
    
    except Exception as e:
        logger.error(f"Error extracting grant URLs: {e}")
        return []
...
def upsert_nysed_grant(grant_data: Dict, processed_data: Dict, session) -> bool:
    ...
    try:
        ...
        if existing_grant and existing_grant.extra and existing_grant.extra.get('content_hash') == content_hash:
        ...
        opportunity.agency = processed_data.get('agency', "New York State Education Department")
        ...
        session.commit()
        logger.info(f"Successfully upserted grant {source_grant_id}: {opportunity.title}")
        return True
...
def main():
    ...
    with flask_app.app_context():
        ...
        try:
            ...
            logger.info(f"Successfully extracted data from {len(all_grants_data)} grants")
            ...
            logger.info(f"NYSED grant scraping completed. Updated {updated_count} grants.")
